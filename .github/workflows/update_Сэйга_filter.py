...
# -*- coding: utf-8 -*-
"""
晴雅广告规则生成Сэйга.txt脚本（移除指定行，版本号跟随上游）
"""
import os
import hashlib
import sys
...
def main():
    """核心逻辑：生成Сэйга.txt，移除指定行，替换自定义头部"""
    # 1. 路径定义
    temp_file = os.path.join(os.getcwd(), "temp_black_filter.txt")
    # ★ 修改输出文件名为 Сэйга.txt
    output_file = os.path.join(os.getcwd(), "Сэйга.txt")
    ...
    # 6. 无变更则复用本地文件
    if local_core_md5 == upstream_core_md5 and os.path.exists(output_file):
...
"""
    ...
    print(f"CURRENT_DATE={upstream_update_time}")
    print(f"✅ Сэйга.txt生成完成：版本 {upstream_ver}，有效规则 {rule_count} 条")
